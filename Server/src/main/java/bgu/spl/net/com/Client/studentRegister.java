...
public class studentRegister extends ClientFace {
	...
	// assuming users can't register on the same terminal
        ...
        if (data.getUser(userName) != null) {
            answer = new Error();
            output = answer.msgForClient("2","User already registered...");
        }
        else if (myUser!=null&&myUser.isActive()) {
            answer = new Error();
            output = answer.msgForClient("2","you can't registered because you are logged in");
        }
        else {
            ...
            output = answer.msgForClient("2","Registered success!");
        }
        return output;
    }
}
